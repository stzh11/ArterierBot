...
async def start_survey_handl(cq: CallbackQuery, state: FSMContext):
    await state.clear()
    await state.update_data(lang=cq.data[5:])
...
async def q17_contact_details_handl(message: Message, state: FSMContext):
    ...
    await message.answer("Спасибо! Мы вернемся с тремя вариантами для обратной связи в течение 72 часов. В рамках бесплатного теста мы предлагаем 5 работ различных художников.")
